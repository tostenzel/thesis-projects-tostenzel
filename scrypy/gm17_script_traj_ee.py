...
traj_list, step_list_traj = trajectory_sample(n_sample_traj, n_inputs, n_levels, seed, normal=False, numeric_zero = numeric_zero)


# Implement sample post-selection and convert elements from [0,1] to stnormal space.
traj_list, _ = select_sample_set_normal(traj_list, n_sample_select, numeric_zero)

# Save post-selected trajectories and steps in input dir.
with open('input/gm17_selected_trajs.pkl', 'wb') as f:
  pickle.dump(traj_list, f)
  
with open('input/gm17_selected_steps_traj.pkl', 'wb') as f:
...
with open('results/gm17_ee_obs_traj.pkl', 'wb') as f:
