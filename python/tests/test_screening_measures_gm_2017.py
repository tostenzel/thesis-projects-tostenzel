...
def test_screening_measures_gm_2017_uncorrelated_parameters():
    ...
    def linear_function(a, b, c, d, e, f, g, h, i, j, *args):
        return co[0]*a + co[1]*b + co[2]*c + co[3]*d + co[4]*e + co[5]*f + co[6]*g + co[7]*h + co[8]*i + co[9]*j
    ...
    assert_array_almost_equal(abs_ee_ind, expected_abs_ee_ind, 0)
    assert_array_almost_equal(abs_ee_full, expected_abs_ee_full, 0)
    assert_array_almost_equal(sd_ee_ind, expected_sd_ee_ind, 0)
    assert_array_almost_equal(sd_ee_full, expected_sd_ee_full, 0)
