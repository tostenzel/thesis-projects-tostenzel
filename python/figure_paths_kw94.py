...
def choice_paths(save=False):
    ...
    for df, ax, model, tuition_subsidy in zip(
        ...
        with sns.axes_style("whitegrid"):
            ...
            shares["edu"].plot(
                ax=ax, legend=True, linewidth=4.0, color=current_palette[3]
            )
            shares["a"].plot(
                ax=ax, linestyle="-.", linewidth=2, color=current_palette[0]
            )
            shares["b"].plot(
                ax=ax, linestyle="-.", linewidth=2, color=current_palette[1]
            )
            shares["home"].plot(
                ax=ax, linestyle="-.", linewidth=2, color=current_palette[2]
            )
