...
def jac_estimation(save=False):
    ...
    assert -1 <= corr_df.values.any() <= 1, "Corrs must be inside [-1,1]"
    ...
    par_df['sd'] = np.sqrt(np.diag(jacobian_cov_matrix))
    par_df['lower'] = par_df['value'] - 2* par_df['sd']
    par_df['upper'] = par_df['value'] + 2* par_df['sd']

    if save is True:
        cov_df.to_pickle("input/cov.uq.pkl")
        corr_df.to_pickle("input/corr.uq.pkl")
        par_df.to_pickle("input/params.uq.pkl")
        # contains 3 fixed respy params
        params_chol.to_pickle("input/base_params.uq.pkl")
    else:
        pass
  
    return par_df, cov_df, corr_df
...
def _chol_indexed_params(params):
    ...
    p_chol_slice.index = p_chol_slice.index.set_levels(p_chol_slice.index.levels[0].str.replace('shocks_sdcorr', 'shocks_chol'), level=0)

    # And secondly we need to convert the second index to respy cholesky format.
    dic = { "sd": "chol", "corr": "chol"}
    for i, j in dic.items():
        p_chol_slice.index = p_chol_slice.index.set_levels(p_chol_slice.index.levels[1].str.replace(i, j), level=1)
