...
row_traj_reordered = np.array([0.1, 0.1, 0.1, 0.9, 0.1])
expected = np.dot((norm.ppf(row_traj_reordered) + mu), np.sqrt(cov))
#assert expected[0] > norm.ppf(row_traj_reordered[0]), "positive correlation with higher value"
#assert expected[1] == norm.ppf(row_traj_reordered[0]), "equal to first parameter but without correlation"
#assert expected[4] < norm.ppf(row_traj_reordered[4]), "negative correlation with higher value"
...
zq = np.dot(z, linalg.inv(Q))
"""I changed the sequence of this matrix multiplication given a test with correlation."""
z_c = np.dot(M, zq)
