...
steps_one = np.array([2 / 3, -2 / 3, -2 / 3, 2 / 3, 2 / 3, 2 / 3])[idx_one]
...
steps_two = np.array([2 / 3, 2 / 3, 2 / 3, -2 / 3, -2 / 3, -2 / 3])[idx_two]
...
steps_three = np.array([-2 / 3, -2 / 3, 2 / 3, -2 / 3, -2 / 3, +2 / 3])[idx_three]
...
ee_sd = np.sqrt((1 / (4 - 1)) * (np.sum((ee_i - ee) ** 2, axis=1).reshape(6, 1)))
...
for i in range(0, 6):
    print(
        (
            sobol_model_three(*traj_three[i + 1, :])
            - sobol_model_three(*traj_three[i, :])
        )
        / steps_three[i]
    )
