...
def trans_ee_ind_trajectories(sample_traj_list, mu, cov):
    ...
    trans_piplusone_i = []
    for traj in range(0, n_traj_sample):
        trans_pi_i.append(inverse_ee_ind_reorder_trajectory(zero_idx_diff[traj], p_i_plus_one=False))
...
def trans_ee_full_trajectories(sample_traj_list, mu, cov):
    ...
    for traj in range(0, n_traj_sample):
        trans_piplusone_iminusone.append(inverse_ee_full_reorder_trajectory(two_idx_diff[traj]))
...
trans_two = trans_ee_full_trajectories(
        sample_traj_list, mu, cov)
