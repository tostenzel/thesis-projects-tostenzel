    ...
    for scenario, idx in zip(scenarios, range(0, 2)):
        with sns.axes_style("whitegrid"):
            ...
            for occ in occupations:
                ...
                means[idx][occ].plot(
                    ax=axs[idx], legend=True, linewidth=1.0, color=current_palette[colour]
                )
                axs[idx].fill_between(means[idx].index,
                        percentile_99[idx][occ], percentile_1[idx][occ],
                        facecolor=current_palette[colour], alpha=0.2)
                colour += 1
