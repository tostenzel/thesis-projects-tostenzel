...
Another level of analysis, and the specific methods are derived from the following three properties of the computational model. These are: First, the input parameters are correlated. Second, 27 input parameters is a moderately large number. Third, the computation time of the computational model is not rapidly short. The second point mainly amplifies the third point. The combination of these properties is also a novelty in the economic literature. Therefore, the approach that is explained in the following is another contribution to this literature.

There are multiple ways to deal with the the moderate computational costs and the parameter dependencies. Each has its own advantages and disadvantages. However, the general approach of this thesis is to circumvent unnecessary degrees of methodical complexity to avoid errors. Thus, I apply the following methods: First, I decrease the number of uncertain input parameters by means of Morris screening as part of a intermediate LSA. This approach is chosen over the alternative of constructing a surrogate model.
...
\newline
In summary, the thesis makes the following contributions. First, it quantifies the parametric uncertainty for a merely microeconomic model. Second, it computes own estimates for this uncertainty. The next points emphasize that the thesis is the first contribution that carries out a GSA for a model with a moderate number of correlated input parameters: Third, it reduces the number of uncertain, correlated input parameters by means of Morris Screening to prepare a subsequent GSA. Fourth, it computes Sobol' indices for correlated input parameters using a quasi-Monte Carlo scheme.
